...
export default async function handler(
  ...
  switch (req.method) {
    case "POST":
      try {
        const { pinCode, repeatPinCode, otp } = req.body; // Extract email and password from request body
        ...
        if (pinCode !== repeatPinCode) {
          return res.status(403).json({
            message: "Pin code and repeat pin code do not match",
          });
        }

        await User.findByIdAndUpdate(userFound.id, {
          withdrawalPin: pinCode,
        });
